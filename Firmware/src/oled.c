...
static const uint8_t font8x8_data[][8] = {
	...
	{0x22, 0x7E, 0x4B, 0x4B, 0x7D, 0x39, 0x00, 0x00},   // '3'
...
{
	static char voltamp_str[] = "00.0V      00.0A";
	static char percent_str[] = "000%";

	// Convert the mV and mA values to the string
	voltamp_str[0] = '0' + (batt_mv / 1000) / 10;
	voltamp_str[1] = '0' + (batt_mv / 1000) % 10;
	voltamp_str[3] = '0' + (batt_mv % 1000) / 100;


//	printf("%s\n", voltamp_str);

	// Draw the modified strings to the framebuffer
	font8x8_to_framebuffer(voltamp_str, (uint8_t *)&_oled_buffer[0][0]);
